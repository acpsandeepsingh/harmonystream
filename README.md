...
The build uses different `BUILD_TARGET` values per job:

- `gh-pages` for website artifacts (uses `/harmonystream` base path)
- `android` for APK web assets (no base path, so JS/buttons/player work inside WebView)
...
- Installs Node + Java + Android SDK dependencies
  - Java 21 is used for Android compilation compatibility
